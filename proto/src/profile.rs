...
pub struct Pprof {
    ...
    known_locations: HashMap<(u64, u64), u64>, // unique location id -> location
    locations: Vec<pprof::Location>,           // maybe we don't need this?
...
impl Pprof {
    ...
    pub fn add_location(&mut self, address: u64, mapping_id: u64, lines: Vec<pprof::Line>) -> u64 {
        ...
        let location = pprof::Location {
            id,
            mapping_id,
            address, // todo: understand how this would play as the id and address are the same
    ...
    pub fn add_sample(&mut self, location_ids: Vec<u64>, count: i64, labels: Vec<pprof::Label>) {
        let sample = pprof::Sample {
            ...
            value: vec![count, count * 1_000_000_000 / self.freq_in_hz], // not sure how this would work, should we add the same count value for each address in a stack?
            label: labels,
        };
...
// TODO
// - [partially done] ensure the profile.proto (pprof) semantics are correct and that various tools such as go.pprof can deal with them
// - [partially done] check google's pprof tooling to ensure we are doing the right thing
// - [ ] aim to allocate as little a possible while generating the pprof (e.g. maps to make things unique etc)
// - [ ] benchmark / profile this code path
// - [ ] the profile, at least on disk "must be gzipped"
// - [ ] compare compression rates with gzip vs zstd
// - [ ] perhaps abstract the "get or insert" logic as it's pretty much the same
// - [ ] the code could be more type safe (not all ids are the same)
...
mod tests {
    ...
    // Using this test to prototype higher level APIs for pprof profile generation.
    #[test]
    fn test_profile() {
        use rand::Rng;
        use std::fs::File;
        ...
        for raw_sample in raw_samples {
            ...
            for addr in raw_sample.0 {
                let mapping_id: u64 = pprof.add_mapping(
                    if addr == 0 { 1 } else { addr }, // ../lightswitch/profile.pb: malformed profile: found mapping with reserved ID=0
                    rng.gen(),
                    rng.gen(),
                    rng.gen(),
