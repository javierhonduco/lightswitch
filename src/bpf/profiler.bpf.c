...
static __always_inline void add_stack(struct bpf_perf_event_data *ctx,
  ...
  stack_count_key_t *stack_key = &unwind_state->raw_stack.stack_key;
  ...
  if (unwind_state->raw_stack.stack.len >= 1) {
    u64 user_stack_id = hash_stack(&unwind_state->raw_stack.stack);
    int err = bpf_map_update_elem(&stacks, &user_stack_id, &unwind_state->raw_stack.stack,
                                  BPF_ANY);
    if (err == 0) {
  ...
  int ret = bpf_get_stack(ctx, unwind_state->raw_stack.stack.addresses, MAX_STACK_DEPTH * sizeof(u64), 0);
  if (ret >= 0) {
    unwind_state->raw_stack.stack.len = ret / sizeof(u64);
  }

  if (unwind_state->raw_stack.stack.len >= 1) {
    u64 kernel_stack_id = hash_stack(&unwind_state->raw_stack.stack);
    int err = bpf_map_update_elem(&stacks, &kernel_stack_id, &unwind_state->raw_stack.stack,
                                  BPF_ANY);
    if (err == 0) {
  ...
  u64 *count = bpf_map_lookup_or_try_init(&aggregated_stacks,
                                           &unwind_state->raw_stack.stack_key, &zero);
  if (count) {
...
static __always_inline bool set_initial_state(unwind_state_t *unwind_state, bpf_user_pt_regs_t *regs) {
  ...
  unwind_state->raw_stack.stack_key.user_stack_id = 0;
  unwind_state->raw_stack.stack_key.kernel_stack_id = 0;
