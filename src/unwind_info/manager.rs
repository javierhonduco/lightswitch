...
/*         println!("{:?}", self.cache_dir);
        ...
        for direntry in fs::read_dir(&self.cache_dir)?.flatten() {
            ...
            println!("{:?} {:?}", name, ExecutableId::from_str(name));
...
mod tests {
    ...
    fn test_custom_usage_ordering() {
        ...
        assert_eq!([&before, &after].iter().max().unwrap().executable_id, ExecutableId(0xBAD));
    ...
    fn test_unwind_info_manager_cache_eviction() {
        ...
        for i in 0..20 {
            File::create(path.join(format!("{:x}FFFFFFFFFFFFFFF", i))).unwrap();
        }
