...
int detect_bpf_features() {
  ...
  // we need to ensure we are running in the user context of the userspace
  // that's loading this program.
  ...
  has_tail_call =
      bpf_core_enum_value_exists(enum bpf_func_id, BPF_FUNC_tail_call);

  has_ringbuf =
      bpf_core_enum_value_exists(enum bpf_map_type, BPF_MAP_TYPE_RINGBUF);

  has_map_of_maps =
      bpf_core_enum_value_exists(enum bpf_map_type, BPF_MAP_TYPE_HASH_OF_MAPS);

  has_batch_map_operations =
      bpf_core_enum_value_exists(enum bpf_cmd, BPF_MAP_LOOKUP_AND_DELETE_BATCH);

  has_task_pt_regs_helper =
  ...
  has_loop = bpf_core_enum_value_exists(enum bpf_func_id, BPF_FUNC_loop);

  has_iter_num_new =
      bpf_core_enum_value_exists(enum special_kfunc_type, KF_bpf_iter_num_new);

  has_iter_num_next =
      bpf_core_enum_value_exists(enum special_kfunc_type, KF_bpf_iter_num_next);

  has_iter_num_destroy = bpf_core_enum_value_exists(enum special_kfunc_type,
                                                    KF_bpf_iter_num_destroy);

  userspace_pid_ns_level =
      BPF_CORE_READ(task, nsproxy, pid_ns_for_children, level);
