...
impl K8sMetadataCache {
    pub fn new(node_name: String) -> Result<Self, kube::Error> {
        let container_to_pod: Cache<String, String> =
            Cache::builder().max_capacity(CONTAINER_CACHE_SIZE).build();
        let pod_metadata: Cache<String, PodMetadata> =
        ...
        let fallback_runtime = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .ok();
        let fallback_api = fallback_runtime
            .as_ref()
            .and_then(|rt| rt.block_on(async { Client::try_default().await.ok().map(Api::all) }));
    ...
    pub fn new_for_test(
        ...
        let deferred_cids = Cache::builder()
            .max_capacity(DEFERRED_CACHE_SIZE)
            .time_to_live(DEFERRED_TTL)
            .build();
    ...
    pub fn get_pod_metadata(&self, container_id: &str) -> Option<PodMetadata> {
        ...
        debug!(
            "cache miss for container id {}, listing pods from apiserver",
            container_id
        );
        ...
        debug!(
            "container id {} not found after list-and-scan, deferring",
            container_id
        );
...
fn pod_to_cache_entry(
    ...
    /*
     * kubectl get pod -n authentik authentik-server-6f794b44fd-mjnf6 -o
     * jsonpath='{.metadata.ownerReferences}' | python3 -m json.tool [
     *       {
     *          "apiVersion": "apps/v1",
...
// Called by run_kubeapi_stream to upsert a PodMetadata on receipt of an
// Apply/InitApply/Delete event from the kube-api watcher.
fn handle_pod_event(
    container_to_pod: &Cache<String, String>,
    ...
    let status = match pod.status.as_ref() {
        ...
        None => {
            warn!(
                "Unable to fetch pod status for pod {:?}, returning",
                metadata.name
            );
            return;
        }
    ...
    let (pod_uid, pod_meta) = match pod_to_cache_entry(metadata, status) {
        ...
        None => {
            warn!(
                "Unable to extract cache entry for pod {:?}, returning",
                metadata.name
            );
            return;
        }
...
//  kubectl get pod -n lightswitch lightswitch-h9dzp -o
...
mod tests {
    ...
    fn test_parse_container_id_no_scheme() {
        assert_eq!(
            parse_container_id("62f1aa03d4a3f1b315941202e240a5f808f760823cce2e329a6285b3a45007b2"),
            None
        );
    }
    ...
    fn test_apply_inserts_container_and_pod() {
        ...
        let pod = make_pod(
            ...
            Some("argocd-application-controller-0"),
            Some("argocd"),
            vec!["containerd://62f1aa03d4a3f1b315941202e240a5f808f760823cce2e329a6285b3a45007b2"],
            vec![],
            Some("StatefulSet"),
            Some("argocd-application-controller"),
        );
    ...
    fn test_apply_inserts_init_containers() {
        ...
        let pod = make_pod(
            ...
            Some("argocd-dex-server-6f7669b5c-n7ffd"),
            Some("argocd"),
            ...
            Some("ReplicaSet"),
            Some("argocd-dex-server-6f7669b5c"),
        );
        handle_pod_event(&cid_cache, &pod_cache, pod, false);
        assert_eq!(
            cid_cache
                .get(
                    &"d701397470f564d3a0f699798d628f617c83b091a78ff519dfb47d96725b0cd6".to_string()
                )
                .unwrap(),
        ...
        assert_eq!(
            cid_cache
                .get(
                    &"a29ea049108b7b145283695fe1124644b52cf28e257eef865229a2227400fc97".to_string()
                )
                .unwrap(),
            "9c154e1e-01fc-4c58-9645-1591ea556275"
        );
    ...
    fn test_apply_with_owner_references() {
        ...
        let pod = make_pod(
            ...
            Some("archive-579664cb57-5tzgb"),
            Some("archive"),
            vec!["containerd://e9bddc1edab79f0e9bdcf34e7db20242c0b59b01db2467c6b05b4a5d2a6873aa"],
            vec![],
            Some("ReplicaSet"),
            Some("archive-579664cb57"),
        );
        handle_pod_event(&cid_cache, &pod_cache, pod, false);
        let meta = pod_cache
            .get(&"b5c6c87a-0744-4447-862f-4de6492154fd".to_string())
            .unwrap();
    ...
    fn test_delete_removes_both_caches() {
        ...
        let pod = make_pod(
            ...
            Some("argocd-redis-65858f5d69-rjq7w"),
            Some("argocd"),
            vec!["containerd://a49f75b30fd21c458e1da2356d1f94b0412aaf92cb2439b2deb631964dce213c"],
            vec![],
            Some("ReplicaSet"),
    ...
    fn test_multiple_containers_same_pod() {
        ...
        let meta = pod_cache
            .get(&"e9e8b2db-8ed0-4235-a094-f24fd76ddc11".to_string())
            .unwrap();
    ...
    // simulate container ID reuse across pod replacement (old pod deleted, new pod
    // gets same cid)
    #[test]
    fn test_apply_overwrites_existing() {
        ...
        let pod1 = make_pod(
            ...
            Some("metallb-speaker-456qs"),
            Some("metallb-system"),
            vec!["containerd://8bf96670fe83a481129eb49b45de412086b9d85e8975ee91bfa91f1d5ccfccc1"],
            vec![],
            Some("DaemonSet"),
            Some("metallb-speaker"),
        );
        ...
        assert_eq!(
            pod_cache
                .get(&"b12b86ec-2a96-44ca-9255-89cf5b83af5c".to_string())
                .unwrap()
                .name,
        ...
        assert_eq!(
            cid_cache
                .get(
                    &"8bf96670fe83a481129eb49b45de412086b9d85e8975ee91bfa91f1d5ccfccc1".to_string()
                )
                .unwrap(),
            "e9e8b2db-8ed0-4235-a094-f24fd76ddc11"
        );
    ...
    fn test_get_pod_metadata_two_level_lookup() {
        ...
        let meta = cache
            .get_pod_metadata("62f1aa03d4a3f1b315941202e240a5f808f760823cce2e329a6285b3a45007b2")
            .unwrap();
        assert_eq!(meta.name, "argocd-application-controller-0");
        assert_eq!(meta.namespace, "argocd");
    }
