...
impl K8sMetadataProvider {
    ...
    fn labels_for_container(&self, container_id: &str) -> Vec<MetadataLabel> {
        let Some(pod_meta) = self.k8_pod_cache.get_pod_metadata(container_id) else {
            debug!(
                "Unable to fetch pod metadata for container id {}",
                container_id
            );
            return vec![];
        };
        ...
        if let Some(owner_kind) = pod_meta.owner_kind {
            labels.push(MetadataLabel::from_string_value(
                K8sLabel::OwnerKind.key().into(),
                owner_kind,
            ));
        }
        if let Some(owner_name) = pod_meta.owner_name {
            labels.push(MetadataLabel::from_string_value(
                K8sLabel::OwnerName.key().into(),
                owner_name,
            ));
        }
        labels
...
mod tests {
    use super::*;
    use crate::containers::k8s_metadata_cache::PodMetadata;
    ...
    fn make_provider(
        ...
        K8sMetadataProvider::new(K8sMetadataCache::new_for_test(
            container_to_pod,
            pod_metadata,
        ))
    }
    ...
    fn test_labels_for_known_container() {
        ...
        let labels = provider.labels_for_container(
        ...
        assert_eq!(
            labels[0].value,
            MetadataLabelValue::String("lightswitch-h9dzp".to_string())
        );
        assert_eq!(labels[1].key, "k8s.namespace.name");
        assert_eq!(
            labels[1].value,
            MetadataLabelValue::String("lightswitch".to_string())
        );
        assert_eq!(labels[2].key, "k8s.owner.kind");
        assert_eq!(
            labels[2].value,
            MetadataLabelValue::String("DaemonSet".to_string())
        );
    ...
    fn test_labels_without_owner() {
        ...
        let labels = provider.labels_for_container(
            "40fc0254a5fc1ec16215e91a33e81496aa2b5a8520fc4deb27bed04d2485f5ea",
        );
