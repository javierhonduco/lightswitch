...
// NOTE: When writing this class, I only have a kube cluster with containerd, as
// well as cgroups v2 on the host to do an actual test against.
// if you have better live examples from nodes that matches that specific setup,
// please update
...
// kubectl exec -n lightswitch lightswitch-h9dzp -- cat /proc/self/cgroup
// 2>/dev/null 0::/kubepods.slice/kubepods-burstable.slice/
// kubepods-burstable-pod516960bd_ad68_47cb_8f70_3343daebe9a4.slice/
// cri-containerd-36bbe89f0d3c9b982f920d6edf290b7083774993cca48b7fcf11e845875e8b4e.
// scope strip .scope at the end
fn strip_scope_suffix(input: &str) -> &str {
    input.strip_suffix(CGROUP_SCOPE_SUFFIX).unwrap_or(input)
}

// assuming that we've gotten the last / here, strip the prefix indicating the
...
fn extract_container_id(pathname: &str) -> Option<String> {
    ...
    if id.len() == 64 && id.chars().all(|c| c.is_ascii_hexdigit()) {
        debug!(
            "extracted container id {} from cgroup path {}",
            id, pathname
        );
        Some(id.to_string())
    } else {
...
mod tests {
    ...
    fn test_strip_runtime_prefix_crio() {
        assert_eq!(
            strip_runtime_prefix(
                "crio-53feacda5b184ce32936690e8bbd17d670114b81a7b7c2abb05dbf966bf9ecc2"
            ),
    ...
    fn test_strip_runtime_prefix_docker() {
        assert_eq!(
            strip_runtime_prefix(
                "docker-3a622e2707cb0a3aa78d528cb0638d22499c7e0ba85e472aef1eb5dc4aca4541"
            ),
